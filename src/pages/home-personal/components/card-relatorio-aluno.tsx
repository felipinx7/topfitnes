...
export function CardRelatorioAluno(data: DataAluno) {
  // estados e variaveis utilizados no componente
  const fotoAluno = BaseUrlFoto(data.foto);

  return (
    <article className="w-full bg-[#F0FCECEF] border border-[#D9D9D9] shadow-[0_4px_20px_0_rgba(55,87,64,0.15)] p-2  rounded-[1rem] flex items-center justify-between">
      ...
      <div className="">
        <button className="bg-verde-100 rounded-[0.6rem] w-auto px-4 py-1 duration-500 ease-in-out transition-all hover:bg-verde-400 cursor-pointer text-white font-[600]">
          ACESSAR
        </button>
      </div>
    </article>
