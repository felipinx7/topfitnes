...
export default function HeaderMobile() {
  ...
  return (
    <header className="min-lg:hidden w-full p-4 px-6 z-100 h-[80px] flex items-center justify-between mt-4">
      {/* Logo */}
      <Link href={"/"}>
      ...
      <button
        ...
        aria-label={toggleMenu ? "Fechar Menu" : "Abrir Menu"}
        className="transition-all ease-in-out z-[99999] duration-75"
      >
        {toggleMenu ? (
      ...
      {/* conteudo do menu */}
      <div
        className={`fixed w-[90%] min-h-[100vh] bg-[#0d0d0d] top-0 right-0 py-40 px-4 text-white flex flex-col items-end gap-4 z-20 ${toggleMenu ? "translate-x-0" : "translate-x-full"} transition-transform duration-300 ease-in-out`}
      >
        ...
        <div className="flex flex-col w-[70%] max-sm:w-full items-center gap-4">
          <button
            ...
            className="border-1 p-4 cursor-pointer w-full rounded-[0.5rem] text-[1rem] ease-in duration-[0.3s] hover:bg-[var(--verde-primario)] font-[600] px-8 border-[var(--verde-primario)]"
          >
            ACESSAR
          </button>
          <button
